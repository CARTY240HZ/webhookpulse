...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // S5: Exclude 'secret' from response. Use 'secret' to determine has_secret (secret_hash column not ready yet).
    ...
    const enriched = (webhooks || []).map((w: Record<string, unknown>) => ({
      ...
      has_secret: !!(w.secret && String(w.secret).trim() !== '' && String(w.secret).trim() !== 'null'),
    }))
